# UI Foundry Spinner Component Generator

Standalone visual Spinner generator built with Next.js, React, and TypeScript.

Loading indicator studio for ring, dot, bar, overlay, inline, labelled, and reduced-motion-safe spinner systems.
...
- app shell and dashboard designers
- developers adding loading states
- buyers who need consistent spinner and busy-state UI
...
The editor is focused on controls that are native to Spinner and useful in real product work:
- renderer family, size, thickness, speed, gap, color, track, and material styling
- inline, overlay, fullscreen, card, and button-adjacent loading presentations
- label, aria-live, aria-busy context, reduced-motion fallback, and accessibility text
- motion/effect controls that remain native to loading indicators
- single React export source for preview, copy, and download

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Spinner.
...
This repo is intended to publish as Abid-Al-Hossain/UI_spinner.
